...
#root = tk.Tk()
#app = Application(master=root,argumen_Canvas_lainnya)
#
#def fungsiLoop():
#    #coding app
#    root.after(10,fungsiLoop)
#
#fungsiLoop()
#root.mainloop()
...
class Application(tk.Canvas):
    def __init__(self, *args, **kwargs):
        ...
        #self.TeksAcak = ["Plastik itu berguna, yakin mau dibuang?",
        #        "Wah plastik botol cantik",
        #        "Hey, itu plastik ya? Bisa didaur ulang lhoo"]
        #self.TextRandom = ["That plastic is useful, are you sure want to trash it?",
        #        "Nice plastic bottle there",
        #        "Hey, is that plastic? It can be reused!"]
    ...
    def _showThankyou(self):
        if self.thanks:
            ...
            #self.randomMessage.config(text='Silakan dengan sukarela tekan tombol "Spasi" difoto')
    ...
    def once(self, camFeed=None, takeData=False):
        #camFeed = cv2.resize(camFeed,(1024,576))
        if __name__ == '__main__':
            ret, camFeed = self.vid.read()
        self._showReminder()
        if takeData:
...
if __name__ == "__main__":
    app = Application("/home/emrc/Third-parties/plastic-detection/logo.jpeg",master=tk.Tk(),width=1280,height=1024,bg='white')
